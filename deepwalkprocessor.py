...
def get_similarity_matrix(adj_matrix , threshold=0.00008):
    # threshold = threshold
    ...
    #         if adj_matrix[i][j] == 1:
    #             adj_matrix[i][j] = int(j + 1)

    # adj_matrix_copy = adj_matrix.tolist()
    # adj_matrix_copy = [[int(value) for value in row if value != 0] for row in adj_matrix_copy]
